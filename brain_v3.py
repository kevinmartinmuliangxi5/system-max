...
# -*- coding: utf-8 -*-
"""
Brain v3.2 - GLM优化版任务理解与蓝图生成模块

集成功能:
- Compound Engineering方法论（GLM-4.7真实AI代理）
- SpecKit规格驱动开发
- draw.io MCP可视化
...
v3.2优化重点:
- Brain层100%使用GLM（不消耗Claude Pro配额）
- 真实的CE代理（req_dev, spec_writer, architect, code_reviewer等）
- 智能路由策略（GLM主力 + Claude Pro关键任务）
"""
...
try:
    ...
    from .ralph.tools.claude_pro_ce_agent import ClaudeProCEAgent
except:
    try:
...
class BrainV3:
    ...
    def __init__(self):
        ...
        # 集成的工具 - Brain使用Claude Pro CE代理（最高质量规划）
        if ClaudeProCEAgent:
            try:
                self.ce_agent = ClaudeProCEAgent()
                print("✓ 已启用Claude Pro CE代理（最高质量规划）")
            except Exception as e:
                print(f"⚠️  Claude Pro CE代理初始化失败: {e}")
                print(f"   错误详情: 请确保设置环境变量 ANTHROPIC_API_KEY")
                print("   降级使用本地CE模拟器")
                self.ce_agent = CompoundEngineeringAgent()
        else:
            print("⚠️  未找到Claude Pro CE代理，使用本地模拟器")
            self.ce_agent = CompoundEngineeringAgent()
    ...
    def print_header(self):
        ce_status = "GLM-4.7 (真实AI)" if isinstance(self.ce_agent, GLMCompoundEngineeringAgent if GLMCompoundEngineeringAgent else type(None)) else "本地模拟"
        print(f'''
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║        🧠 Brain v3.2 - GLM优化版任务规划系统                     ║
║                                                                  ║
║        集成功能:                                                  ║
║        • Compound Engineering ({ce_status})                      ║
║        • SpecKit 规格驱动开发                                     ║
║        • draw.io 可视化生成                                       ║
║        • 双记忆系统检索                                           ║
║                                                                  ║
║        配额优化: Brain使用GLM，不消耗Claude Pro配额               ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
        ''')
