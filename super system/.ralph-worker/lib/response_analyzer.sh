...
parse_json_response() {
    ...
    # Base confidence from actual work indicators, not fixed values

    # Start with structured response boost
    local base_confidence=0
    if [[ "$has_result_field" == "true" ]]; then
        base_confidence=30  # Structured response foundation
    fi

    # Calculate work-based confidence boost
    local work_confidence=0

    # 1. Files modified (strongest signal of real work)
    if [[ $files_modified -gt 0 ]]; then
        # Scale: 1 file = +10, 5 files = +30, 10+ files = +40
        if [[ $files_modified -ge 10 ]]; then
            work_confidence=$((work_confidence + 40))
        elif [[ $files_modified -ge 5 ]]; then
            work_confidence=$((work_confidence + 30))
        elif [[ $files_modified -ge 3 ]]; then
            work_confidence=$((work_confidence + 20))
        else
            work_confidence=$((work_confidence + files_modified * 10))
        fi
    fi

    # 2. Progress indicators from metadata
    if [[ $progress_count -gt 0 ]]; then
        work_confidence=$((work_confidence + progress_count * 8))
    fi

    # 3. Check for tool usage in output (Read, Write, Edit, Bash)
    # Count tool invocations as a proxy for work done
    local tool_count=0
    if [[ -f "$output_file" ]]; then
        # Count tool_use events in stream-json format
        tool_count=$(jq '[.[] | select(.type == "stream_event" and .event.type == "content_block_start" and .event.content_block.type == "tool_use")] | length' "$output_file" 2>/dev/null || echo "0")
        tool_count=$((tool_count + 0))  # Ensure integer

        if [[ $tool_count -gt 0 ]]; then
            # Scale: 1-3 tools = +5 each, 4-10 tools = +3 each (capped at +30)
            if [[ $tool_count -le 3 ]]; then
                work_confidence=$((work_confidence + tool_count * 5))
            else
                work_confidence=$((work_confidence + 15 + (tool_count - 3) * 3))
            fi
            # Cap at +30 for tool usage
            if [[ $work_confidence -gt 60 ]]; then
                work_confidence=60
            fi
        fi
    fi

    # 4. Output length as indicator of substantive work
    local output_length=0
    if [[ -f "$output_file" ]]; then
        local result_text=$(jq -r '.result // ""' "$output_file" 2>/dev/null)
        output_length=${#result_text}

        if [[ $output_length -gt 2000 ]]; then
            work_confidence=$((work_confidence + 15))  # Substantial output
        elif [[ $output_length -gt 500 ]]; then
            work_confidence=$((work_confidence + 10))  # Moderate output
        elif [[ $output_length -gt 100 ]]; then
            work_confidence=$((work_confidence + 5))   # Some output
        fi
    fi

    # Combine base + work confidence (cap at 95 to leave room for completion boost)
    confidence=$((base_confidence + work_confidence))
    if [[ $confidence -gt 95 ]]; then
        confidence=95
    fi

    # Minimum confidence for any valid response
