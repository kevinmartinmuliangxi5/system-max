#!/bin/bash
# Ralph Auto Mode with Real-time Streaming + Interrupt
# 自动迭代 + 实时流式输出 + 随时可打断

echo "╔════════════════════════════════════════════════════════════╗"
echo "║  Ralph Auto Stream Mode - 实时流式输出                    ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
echo "💡 模式说明："
echo "  • Worker自动连续迭代"
echo "  • 实时显示Worker的每一步输出（流式）"
echo "  • 每轮结束后5秒倒计时"
...
while [ $loop_count -lt $max_loops ]; do
    ...
    # ----------------------------------------
    # Step 3: 执行Worker（实时流式输出）
    # ----------------------------------------
    echo "🤖 Worker执行中（智普API）- 实时输出："
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""

    log_file=".ralph/logs/loop_${loop_count}.log"

    # 关键：使用 stdbuf 禁用缓冲 + tee 实时显示并保存日志
    # 添加 --dangerously-disable-sandbox 禁用沙盒模式，允许完全文件访问
    # 如果 stdbuf 不可用，则使用 script -q（MacOS）或直接使用 tee
    if command -v stdbuf &> /dev/null; then
        cat .ralph/current_instruction.txt | stdbuf -oL -eL claude --dangerously-skip-permissions 2>&1 | tee "$log_file"
    elif command -v script &> /dev/null && [[ "$OSTYPE" == "darwin"* ]]; then
        cat .ralph/current_instruction.txt | script -q /dev/null claude --dangerously-skip-permissions 2>&1 | tee "$log_file"
    else
        # 直接使用 tee（可能有轻微延迟）
